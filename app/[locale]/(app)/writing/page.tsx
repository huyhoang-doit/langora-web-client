...
import { MOCK_WRITING_EXERCISES, MOCK_WRITING_TOPICS } from "@/lib/mock-data/writing";
...
export default function WritingHubPage() {
  ...
  const filteredExercises = activeTopic 
    ? MOCK_WRITING_EXERCISES.filter(ex => ex.topicId === activeTopic)
    : MOCK_WRITING_EXERCISES;
    ...
    <div className="flex flex-col h-full overflow-hidden bg-background" id="writing-hub-page">
      ...
      <div className="flex-grow overflow-y-auto p-6 scrollbar-thin">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          {/* Exercises Grid */}
          <div className="space-y-3">
            <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Available Exercises ({filteredExercises.length})</h3>
                ...
                <div key={ex.id} className="card-edu card-edu-interactive p-6 bg-card flex flex-col h-full relative overflow-hidden group">
                  ...
                  <div className="flex-grow space-y-4 relative z-10">
                    <div className="flex justify-between items-start">
                      ...
                      <Badge variant="outline" className="bg-background border-2 font-bold">
                        {ex.level}
                      </Badge>
                    </div>
                    
                    <div>
                      <h4 className="text-lg font-black text-foreground text-heading line-clamp-1">{ex.title}</h4>
